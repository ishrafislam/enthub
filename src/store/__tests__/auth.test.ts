...
describe('authStore', () => {
  ...
  describe('login', () => {
    ...
    it('should persist userId to localStorage', async () => {
      ...
      expect(localStorageMock.setItem).toHaveBeenCalledWith('enthub_user_id', 'user-xyz-789')
    })
    ...
    it('should overwrite previous userId', async () => {
      ...
      expect(localStorageMock.setItem).toHaveBeenLastCalledWith('enthub_user_id', 'user-second')
    })
  })
  ...
  describe('logout', () => {
    ...
    it('should remove userId from localStorage', async () => {
      ...
      expect(localStorageMock.removeItem).toHaveBeenCalledWith('enthub_user_id')
    })
  ...
  describe('cross-tab synchronization', () => {
    it('should update userId when storage event fires for correct key', async () => {
      ...
      storageHandler!({ key: 'enthub_user_id', newValue: 'new-tab-user' } as StorageEvent)
    ...
    it('should not update userId when storage event is for different key', async () => {
      ...
      storageHandler!({ key: 'other_key', newValue: 'some-value' } as StorageEvent)
    ...
    it('should handle null newValue (logout from another tab)', async () => {
      ...
      storageHandler!({ key: 'enthub_user_id', newValue: null } as StorageEvent)
