...
const {
  ...
  hasActiveFilters,
  clearFilters,
} = useListFilters(watchlist, "addedAt");
</script>
    ...
    >
      ...
      >
        <span v-if="hasActiveFilters">
          {{ filteredAndSorted.length }} / {{ watchlist.length }} items
        </span>
        <span v-else>{{ watchlist.length }} items</span>
      </span>
    </div>

    <!-- Controls bar -->
    <div v-if="watchlist && watchlist.length > 0" class="mb-6 flex flex-col sm:flex-row sm:items-center gap-3">
      <!-- Type filter buttons -->
      <div
        :class="[
          'flex shrink-0',
