...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex items-center justify-between mb-8 border-b border-gray-200 dark:border-gray-800 pb-4">
      <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Your Watchlist</h1>
      <span v-if="watchlist" class="bg-teal-500 text-white px-3 py-1 rounded-full text-sm font-bold">
    ...
    <div v-else-if="!watchlist || watchlist.length === 0" class="text-center py-20">
      <div class="inline-flex items-center justify-center w-20 h-20 rounded-full bg-gray-100 dark:bg-gray-800 mb-6">
        <svg class="w-10 h-10 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path></svg>
      </div>
      <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">Your watchlist is empty</h2>
      <p class="text-gray-500 dark:text-gray-400">Start adding movies and shows you want to watch!</p>
      <router-link to="/" class="mt-6 inline-block bg-teal-500 text-white px-6 py-2 rounded-full font-bold hover:bg-teal-600 transition">
        Discover Content
      </router-link>
    </div>
